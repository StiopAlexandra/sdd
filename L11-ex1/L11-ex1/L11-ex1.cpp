...
void AfisareCoada()
{
	...
	while (r!=sfCP)
	{
		cout << r->cheie<< "(" << r->prt << " , "<< r->par->cheie <<") \t";
		r = r->urmCP;
	}
}
		...
		if (p->marc == nemarcat) {
			...
			while (incCP->urmCP != sfCP) {//prelucrarea cozii
				...
				if (q->par) {//afisarea arcului si actualizarea costului total
					cout << "alegem arcul (" << q->par->cheie << "," << q->cheie << ")" << "\n";//afis. arc
					costTotal += q->prt;
				}
				q->marc = marcat;
...
int main() {
	...
	fclose(f);
	AfisImplGraf();
	cout << "\n\nStructura de tip coada -nod(cost,parinte)- urmata de muchiile arborelui de acoperire minim : \n";
	CautPr(incG);
	cout << endl;
	cout << "Costul total este: " << costTotal;
	_getch();
	delete incCP;
	delete sfCP;
	StergGraf();
